...
@section('title',"SIITA  - Reportes de Asesorías")
		...
		<div class="row align-items-end">
			<div class="col-lg-8">
				<div class="page-header-title">
					...
					<div class="d-inline">
						<h4 style="text-transform: none;">Reportes de Asesorías</h4>
			...
			<div class="col-lg-4">
				<div class="page-header-breadcrumb">
					<ul class="breadcrumb-title">
						...
						<li class="breadcrumb-item">Reportes de Asesorías
						</li>
					</ul>
				</div>
			</div>
			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-block">
						...
						<form id="form" method="POST" action="{{ route('reports.get_asesoria_report') }}">
							{!! csrf_field() !!}
							...
							<div class="row">
								...
								<div class="row">
									...
									<div class="col-sm-12 col-xl-6">
										<p>Clase: </p>
										<select id="class_id" name="class_id" class="select2_basic" title="Selecciones una clase">
											<option value="">Todos</option>
											@foreach ($students_classes as $stdcl)
												<option value="{{ $stdcl->id }}" {{ (old("class_id") == $stdcl->id ? "selected":"") }}>{{ $stdcl->name }}</option>
											@endforeach
										</select>
									</div>
									<div class="col-sm-12 col-xl-3">
										<p>Tipo de Asesoría: </p>
										<select id="asesoria_id" name="asesoria_id" class="select2_basic" title="Seleccione el tipo de asesoría">
											<option value="">Todos</option>
											<option value="{{ 1 }}" {{ (old("asesoria_id") == 1 ? "selected":"") }}>Individual</option>
											<option value="{{ 2 }}" {{ (old("asesoria_id") == 2 ? "selected":"") }}>Grupal</option>
										</select>
									</div>
								</div>
							...
							<div class="row">
								<div class="col-sm-12 col-xl-3">
									<p>Estado de Asesoría: </p>
									<select id="asesoria_state" name="asesoria_state" class="select2_basic" title="Selecciones el estado de la asesoría">
										<option value="-1" selected>Todos</option>
										<option value="{{ 0 }}" {{ (old("asesoria_state") == 0 ? "selected":"") }}>Sin terminar</option>
										<option value="{{ 1 }}" {{ (old("asesoria_state") == 1 ? "selected":"") }}>Terminada sin Confirmación</option>
										<option value="{{ 2 }}" {{ (old("asesoria_state") == 2 ? "selected":"") }}>Confirmada</option>
									</select>
								</div>
								<div class="col-sm-12 col-xl-6">
									<p>Rango de Fecha: </p>
							...
							<table style="width:100%;" id="datatable_reports" class="table table-striped table-bordered">
								@if ((!empty($asesorias)))
									<thead id="table_header">
										<tr>
											...
											<th scope="col" style="width: 20%">Profesor</th>
											<th scope="col" style="width: 15%">Alumno</th>
											<th scope="col" style="width: 5%">Carrera</th>
								...
								<tfoot>
									<tr id="table_footer">
										...
										<th style="padding-right: 2.8%" scope="col">Profesor</th>
										<th style="padding-right: 2.8%" scope="col">Alumno</th>
										<th style="padding-right: 2.8%" scope="col">Carrera</th>
										<th style="padding-right: 2.8%" scope="col">Materia</th>
										<th style="padding-right: 2.8%" scope="col">Tipo</th>
										<th style="padding-right: 2.8%" scope="col">Estado</th>
...
<script>
	...
	$(document).ready(function() {
		...
		var jsFileNameDate = 'Bitácora_Asesorías_'+date.getDate()+'_'+month+'_'+date.getFullYear();
									...
									doc['header']=(function() {
										...
										if ($("#start").val()!= '' && $("#end").val()!= '') {
											if (actual_text == "Todos") {
												...
												message2 = "Profesor: " + actual_text;
											}
											message = "Del " + $("#start").val() + " al " + $("#end").val();
											message = message.toString();
											subheader = " - Concentrado Mensual";
											margin1 = 5;
											margin2 = 10;
										...
										return {
											columns: [
			                                    ...
			                                    },
												{
													alignment: 'left',
													italics: false,
													stack: ['BITÁCORA DE ASISTENCIA DE ASESORÍAS ACADÉMICAS'+subheader,
									...
									doc['footer']=(function(page, pages) {
										return {
											columns: [
												...
												{
													alignment: 'right',
													text: ['Asesorías Individuales: ' + cant_asesorias_individuales],
													margin: [20,20]
												},
												{
													alignment: 'right',
													text: ['Asesorías Grupales: ' + cant_asesorias_grupales],
													margin: [20,20]
												},
												{
													alignment: 'right',
													text: ['Total de Asesorías: ' + (cant_asesorias_individuales+cant_asesorias_grupales)],
													margin: [20,20]
												},
												{
													alignment: 'right',
	...
	$("#asesoria_state").change(function() {
		@if (Auth::user()->type != 4 && Auth::user()->type != 5)
			actual_text = $("#professor_id").select2('data')[0]['text'];
		@else
			actual_text = $("#professor_id").text();
	...
	function fillDataTable(is_click=0){
		$("#cant_a_individuales").text("0");
		$("#cant_a_grupales").text("0");
		$("#cant_asesorias").text("0");
		$.ajaxSetup({
			headers: {
		...
		//por la funcion
		$.ajax({
			url: '{{ route('reports.get_asesoria_report') }}',
			method: 'POST',
			data: {
				...
				asesoria_id: $("#asesoria_id").val(),
				asesoria_state: $("#asesoria_state").val(),
				start: $("#start").val(),
				end: $("#end").val(),
			},
			...
			success: function(result) {
				...
				if(asesoria.length==0){
					...
					cant_asesorias_individuales=0;
					cant_asesorias_grupales=0;
					dt_reports.clear().draw();
					for (var i = 0; i < asesoria.length; i++) {
						switch(asesoria[i]["type_asesoria"]){
							case 0:
								asesoria[i]["type_asesoria"]="Sin Asignar";
								break;
							case 1:
								asesoria[i]["type_asesoria"]="Individual";
								cant_asesorias_individuales=cant_asesorias_individuales+1;
								break;
							case 2:
								asesoria[i]["type_asesoria"]="Grupal";
								cant_asesorias_grupales=cant_asesorias_grupales+1;
								break;
						}

						switch(asesoria[i]["state"]){
							case 0:
								asesoria[i]["state"]="Sin Realizar";
								break;
							case 1:
								asesoria[i]["state"]="Terminada sin Confirmación";
								break;
							case 2:
								asesoria[i]["state"]="Confirmada";
								break;
						}

						if (asesoria[i]["professorTitle"] != null) {
							asesoria[i]["professorTitle"] = asesoria[i]["professorTitle"];
						} else if (asesoria[i]["professorTitle"] == null) {
							asesoria[i]["professorTitle"] = "";
						}

						dt_reports.row.add([
								asesoria[i]["id_asesoria"],
								asesoria[i]["professorTitle"] + " " +asesoria[i]["professorFirstName"] + " " + asesoria[i]["professorLastName"] + " " + asesoria[i]["professorSecondLastName"],
								asesoria[i]["university_id"]+" - "+asesoria[i]["first_name"] + " " + asesoria[i]["last_name"] + " " + asesoria[i]["second_last_name"],
								asesoria[i]["name"],
								asesoria[i]["materiaName"],
								asesoria[i]["type_asesoria"],
								asesoria[i]["state"],
								asesoria[i]["date_attention"]
						]).draw();
					}
					$("#cant_a_individuales").text(cant_asesorias_individuales);
					$("#cant_a_grupales").text(cant_asesorias_grupales);
					$("#cant_asesorias").text(cant_asesorias_grupales+cant_asesorias_individuales);
				}
			}
		});
	}
