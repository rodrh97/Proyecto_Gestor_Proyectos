...
@section('title',"SIITA - Reportes de Solicitudes")
		...
		<div class="row align-items-end">
			<div class="col-lg-8">
				<div class="page-header-title">
					...
					<div class="d-inline">
						<h4 style="text-transform: none;">Reportes de Solicitudes</h4>
			...
			<div class="col-lg-4">
				<div class="page-header-breadcrumb">
					<ul class="breadcrumb-title">
						...
						<li class="breadcrumb-item">Reportes de Solicitudes
						</li>
					</ul>
				</div>
			</div>
	...
	<div class="page-body">
		<div class="row">
						...
						<h4 class="sub-title"><i class="icofont icofont-filter"></i> Filtros</h4>
							...
							<div class="row">
								@if (Auth::user()->type == 6)
									<div class="col-sm-12 col-xl-4">
										...
										<select id="attention" name="attention" class="select2_basic" title="Seleccione el tipo de canalización">
											@foreach ($uses_salud as $uses)
												<option value="{{ $uses->id }}" {{ (old("attention") == $uses->id ? "selected":"") }}>{{ $uses->name }}</option>
											@endforeach
										</select>
									</div>
								@elseif (Auth::user()->type == 7)
									<div class="col-sm-12 col-xl-4">
										<p>Tipo de Canalización: </p>
										<select id="attention" name="attention" class="select2_basic" title="Seleccione el tipo de canalización">
											@foreach ($uses_psicologia as $uses)
												<option value="{{ $uses->id }}" {{ (old("attention") == $uses->id ? "selected":"") }}>{{ $uses->name }}</option>
											@endforeach
										</select>
									</div>
								...
								<div class="col-sm-12 col-xl-2">
									<p>Estado Canalización: </p>
									<select id="canalization_state" name="canalization_state" class="select2_basic" title="Seleccione el estado de la canalización">
										<option value="-1" selected>Todos</option>
										<option value="{{ 0 }}" {{ (old("canalization_state") == 0 ? "selected":"") }}>Sin Realizar</option>
										<option value="{{ 1 }}" {{ (old("canalization_state") == 1 ? "selected":"") }}>Terminada sin Confirmación</option>
										<option value="{{ 2 }}" {{ (old("canalization_state") == 2 ? "selected":"") }}>Confirmada</option>
										<option value="{{ 3 }}" {{ (old("canalization_state") == 3 ? "selected":"") }}>Rechazada</option>
									</select>
								</div>
								...
								@if (Auth::user()->type == 6)
									<div class="col-sm-12 col-xl-4">
										...
										<select id="problem" name="problem" class="select2_basic" title="Seleccione el tipo de problema">
											@foreach ($use_salud as $use_s)
												<option value="{{ $use_s->id }}" {{ (old("problem") == $use_s->id ? "selected":"") }}>{{ $use_s->name }}</option>
											@endforeach
										</select>
									</div>
								@elseif (Auth::user()->type == 7)
									<div class="col-sm-12 col-xl-4">
										<p>Tipo de Problema: </p>
										<select id="problem" name="problem" class="select2_basic" title="Seleccione el tipo de problema">
											@foreach ($use_psicologia as $use_p)
												<option value="{{ $use_p->id }}" {{ (old("problem") == $use_p->id ? "selected":"") }}>{{ $use_p->name }}</option>
											@endforeach
										</select>
									</div>
								@endif
							...
							<div class="row">
								<div class="col-sm-12 col-xl-2">
									...
									<select id="tutoria_state" name="tutoria_state" class="select2_basic" title="Seleccione el estado de la tutoría">
										<option value="-1" selected>Todos</option>
										<option value="{{ 0 }}" {{ (old("tutoria_state") == 0 ? "selected":"") }}>Sin Realizar</option>
			...
			<div id="results_section" class="col-sm-12">
				<div class="card">
					<div class="card-block">
						<h4 class="sub-title"><i class="fas fa-list-ol"></i> Totales de Solicitudes</h4>
						<div class="row">
							<div class="col-sm-4">
								<div class="form-group row">
									<label class="col-sm-9 col-form-label" for="username">Solicitudes de Tutorias Individuales:</label>
									<div class="col-sm-3">
										<strong>
							...
							<div class="col-sm-4">
								<div class="form-group row">
									<label class="col-sm-9 col-form-label" for="username">Solicitudes de Tutorias Grupales:</label>
									<div class="col-sm-3">
										<strong>
							...
							<div class="col-sm-4">
								<div class="form-group row">
									<label class="col-sm-9 col-form-label" for="username">Total de Solicitudes:</label>
									<div class="col-sm-3">
										<strong>
							...
							<table style="width:100%;" id="datatable_reports" class="table table-striped table-bordered">
								@if ((!empty($asesorias)))
									<thead id="table_header">
										<tr>
											...
											<th scope="col" style="width: 15%">Estado de Canalizacion</th>
								...
								<tfoot>
									<tr id="table_footer">
										...
										<th style="padding-right: 1.5%" scope="col">Estado de Canalizacion</th>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#canalization_state").val('-1').change();
		...
		var jsFileNameDate = 'Bitácora_Solicitudes_'+date.getDate()+'_'+month+'_'+date.getFullYear();
								...
								customize: function (doc) {
									...
									doc['header']=(function() {
										...
										if ($("#start").val()!= '' && $("#end").val()!= '') {
											...
											subheader = " - Concentrado Mensual de Solicitudes";
											margin1 = 5;
											margin2 = 10;
										...
										return {
											columns: [
			                                    ...
			                                    },
												{
													alignment: 'left',
													italics: false,
													stack: ['BITÁCORA DE SOLICITUDES'+subheader,
									...
									doc['footer']=(function(page, pages) {
										return {
											columns: [
												...
												{
													alignment: 'right',
													text: ['Solicitudes Individuales: ' + cant_tutorias_individuales],
													margin: [20,20]
												},
												{
													alignment: 'right',
													text: ['Solicitudes Grupales: ' + cant_tutorias_grupales],
													margin: [20,20]
												},
												{
													alignment: 'right',
													text: ['Total de Solicitudes: ' + (cant_tutorias_grupales+cant_tutorias_individuales)],
													margin: [20,20]
												},
												{
													alignment: 'right',
	...
	$("#canalization_state").change(function(){
		actual_text = $("#professor_id").select2('data')[0]['text'];
		fillDataTable();
	});
	...
	function fillDataTable(is_click=0){
		...
		//por la funcion
		$.ajax({
			url: '{{ route('reports.get_solicitud_report') }}',
			method: 'POST',
			data: {
				...
				canalization_state: $("#canalization_state").val(),
			...
			success: function(result) {
				...
				if (tutoria.length === 0) {
					...
					for (var i = 0; i < tutoria.length; i++) {
						...
						switch(tutoria[i]["id_attention_problems"]){
							case 1:
								tutoria[i]["id_attention_problems"]="Económico";
								break;
							case 2:
								tutoria[i]["id_attention_problems"]="Personal";
								break;
							case 3:
								tutoria[i]["id_attention_problems"]="Inseguridad";
								break;
							case 4:
								tutoria[i]["id_attention_problems"]="Sexualidad";
								break;
							case 5:
								tutoria[i]["id_attention_problems"]="Transporte";
								break;
							case 6:
								tutoria[i]["id_attention_problems"]="Otro";
								break;
							case 7:
								tutoria[i]["id_attention_problems"]="Salud";
								break;
							case 8:
								tutoria[i]["id_attention_problems"]="Psicología";
								break;
						}
						...
						switch(tutoria[i]["canalization_state"]){
							case 0:
								tutoria[i]["canalization_state"]="Sin Realizar";
								break;
							case 1:
								tutoria[i]["canalization_state"]="Terminada sin Confirmación";
								break;
							case 2:
								tutoria[i]["canalization_state"]="Confirmada";
								break;
							case 3:
								tutoria[i]["canalization_state"]="Rechazada";
								break;
						}
						...
						dt_reports.row.add([
							...
							tutoria[i]["canalization_state"],
							tutoria[i]["id_attention_problems"],
