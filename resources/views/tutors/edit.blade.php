...
@section('title',"Bolsa de Trabajo - Editar Tutor: {$tutor->id}")
		...
		<div class="row align-items-end">
			<div class="col-lg-8">
				<div class="page-header-title">
					...
					<div class="d-inline">
						<h4 style="text-transform: none;">Editar el Tutor con Número de Empleado: {{ $tutor->university_id }}</h4>
						<span style="text-transform: none;">Ingrese los valores solicitados en la parte inferior para editar el tutor seleccionado.</span>
					</div>
				</div>
			</div>
			<div class="col-lg-4">
				<div class="page-header-breadcrumb">
					<ul class="breadcrumb-title">
						...
						<li class="breadcrumb-item">Modifcar Tutor
						</li>
					</ul>
				</div>
			</div>
			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-block">
						<form id="form" method="POST" action="{{ url("tutors/{$tutor->id}") }}" files="true" enctype="multipart/form-data">
							{{ method_field('PUT') }}
							{!! csrf_field() !!}

							<input hidden="true" type="text" id="user_id" name="user_id" value="{{ $tutor->id }}">

							<div class="form-group row">
								<label class="col-sm-2 col-form-label" for="">ID Tutor:</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="" name="" disabled title="Id del Tutor" value="{{ $tutor->id }}">
								</div>
							</div>
							<div class="form-group row">
								<label class="col-sm-2 col-form-label" for="university_id">Numero Empleado:</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="university_id" name="university_id" placeholder="Ej. 100" title="Número de Empleado del Tutor" value="{{ old('university_id')==""?$tutor->university_id: old('university_id') }}">
							...
							<div class="form-group row">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="title" name="title" placeholder="Ej. Dr." title="Grado de Titulación del Tutor" value="{{ old('title')==""?$tutor->title: old('title') }}" title="Titulo o Grado">
							...
							<div class="form-group row">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="name" name="name" placeholder="Ej. Daniel Alejandro" title="Nombre del Tutor" value="{{ old('name')==""?$tutor->first_name: old('name') }}" title="Nombre o nombres del alumno">
									@if ($errors->has('name'))
							...
							<div class="form-group row">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="last_name" name="last_name" placeholder="Ej. López" title="Apellido Paterno del Tutor" value="{{ old('last_name')==""?$tutor->last_name: old('last_name') }}" title="Apellido paterno del alumno">
							...
							<div class="form-group row">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="second_last_name" name="second_last_name" placeholder="Ej. López" title="Apellido Materno del Tutor" value="{{ old('second_last_name')==""?$tutor->second_last_name: old('second_last_name') }}" title="Apellido materno del alumno">
							...
							<div class="form-group row">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="username" name="username" placeholder="Ej. dlopezl" title="Username del Tutor" value="{{ old('username')==""?$tutor->username: old('username') }}" >
									...
									<select id="career_id" name="career_id" class="select2_basic">
										@foreach ($careers as $career)
											@if(old('career_id')!="")
												...
												@endif
											@else
												@if ($tutor->tutor_career_id == $career->id)
													<option selected value='{{ $career->id }}'>{{ $career->name }}</option>
												@else
													<option value='{{ $career->id }}'>{{ $career->name }}</option>
												@endif
							...
							<div class="form-group row">
								...
								<div class="col-sm-3">
									<img id="modal_img" style="border-radius: 15px; margin-top: 0px;width:100%;" src='{{ asset($tutor->image_url)}}' alt="{{ $tutor->title }} {{ $tutor->first_name }} {{ $tutor->last_name }} {{ $tutor->second_last_name }}" class="img-fluid p-b-10">
									<input type="text" name="image_2" class="form-control" hidden value="{{ $tutor->image_url }}">
